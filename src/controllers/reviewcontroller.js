...
const reviewCreate = async function (req, res) {
    try {
        ...
        if (Object.entries(data).length == 0) {
          ...
          if(!(data.rating>0 && data.rating<=5)) return res.status(400).send({ status: false, msg: "rating is not valid" })
...
const reviewUpdate = async function (req, res) {
    try {
        ...
        if (!(await bookModel.findOne({_id:req.params.bookId,isDeleted: false }))) return res.status(400).send({ status: false, msg: "book is not present" })

        if (!(await reviewModel.findOne({_id:req.params.reviewId,isDeleted: false }))) return res.status(400).send({ status: false, msg: "review is not present" })
        if(!(data.rating>0 && data.rating<=5)) return res.status(400).send({ status: false, msg: "rating is not valid" })
        let savedData = await reviewModel.findOneAndUpdate({ _id: req.params.reviewId }, data,{ updatedAt:new Date(),new: true })
...
const reviewDelete = async function (req, res) {
    try {
        ...
        let deletedreview = await reviewModel.findOneAndUpdate({ _id: req.params.reviewId}, { isDeleted:true, deletedAt:Date.now()}, { new: true });
